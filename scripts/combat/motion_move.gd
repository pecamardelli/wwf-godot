...
## later indices are progressively older. masks[k] are bits to IGNORE; a buffer entry
## matches step k when (entry.code & ~masks[k]) == values[k]  (mask = bits to IGNORE).
## The whole motion must complete within max_ticks ARCADE ticks (converted to frames at match time).
